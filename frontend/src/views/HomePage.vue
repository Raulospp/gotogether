      ...
      <div v-if="activeTab === 'schedule'" class="tab-content">
        <div v-for="dia in dias" :key="dia.key" class="day-card">
          ...
          <div v-if="openDias.schedule[dia.key]" class="day-card-body">
            ...
            <button class="btn-save-sm" :class="{ saved: savedDia === dia.key }" @click="saveAndClose('schedule', dia.key)"><ion-icon :icon="savedDia === dia.key ? checkmarkCircleOutline : checkmarkOutline" /> {{ savedDia === dia.key ? '¡Guardado!' : 'Guardar' }}</button>
          </div>
        </div>
      </div>
...
const savedDia = ref('');
function saveAndClose(tab: string, key: string) {
  if (tab === 'schedule') saveSchedule();
  else saveRoutes();
  savedDia.value = key;
  setTimeout(() => {
