  ...
  <ion-page>
    <ion-content :fullscreen="true" class="register-content">
      ...
      <div class="register-wrapper">
        ...
        <h1 class="page-title">REGISTRO – <span class="title-accent">USUARIO</span></h1>
        <div class="form-card">
...
async function handleSubmit() {
  const { name, phone, email, university, city, password } = form.value;
  if (!name || !phone || !email || !university || !city || !password) {
  ...
  loading.value = true;
  try {
    await authStore.registerPasajero({
      name, email, password,
      city, university,
      route: '',
    });
...
.bg-glow-top { position: fixed; width: 300px; height: 300px; background: radial-gradient(circle, rgba(65,88,208,0.3), transparent 70%); filter: blur(60px); top: -80px; left: -60px; pointer-events: none; z-index: 0; }
...
.title-accent { background: linear-gradient(90deg, #4158d0, #6a7de8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
...
.input-wrap.focused { border-color: #4158d0; box-shadow: 0 0 0 3px rgba(65,88,208,0.2); }
...
.btn-submit { width: 100%; padding: 15px; background: linear-gradient(135deg, #4158d0, #2d3db0); border: none; border-radius: 12px; color: #fff; font-family: 'Exo 2', sans-serif; font-size: 16px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 24px rgba(65,88,208,0.45); transition: transform 0.15s; margin-top: 6px; }
