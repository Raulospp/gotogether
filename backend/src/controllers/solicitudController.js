...
exports.crearSolicitud = async (req, res, next) => {
  try {
      ...
      const existe = await pool.query(
        `SELECT id FROM solicitudes
         WHERE pasajero_id = $1 AND conductor_id = $2
           AND estado IN ('pendiente','aceptada','en_curso') AND fecha_viaje = CURRENT_DATE`,
        [userId, conductor_id]
      );
    ...
    if (userRole === 'conductor') {
      ...
      if (existe.rows.length > 0) return res.status(409).json({ message: 'Ya enviaste una invitacion a este pasajero hoy' });
      const result = await pool.query(
        'INSERT INTO solicitudes (pasajero_id, conductor_id, iniciado_por, fecha_viaje) VALUES ($1, $2, $3, CURRENT_DATE) RETURNING *',
        [pasajero_id, userId, userId]
      );
      return res.status(201).json({ message: 'Invitacion enviada', solicitud: result.rows[0] });
    }
