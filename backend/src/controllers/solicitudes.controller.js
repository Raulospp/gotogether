...
export async function getPendientesCount(req, res, next) {
  try {
    const { rows } = await pool.query(
...
export async function getMisSolicitudes(req, res, next) {
  try {
    const { rows } = await pool.query(`
      SELECT
...
export async function guardarPickup(req, res, next) {
  try {
    const {
